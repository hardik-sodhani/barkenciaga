...
const routes = [
  { path: "/", contains: "Barkenciaga", alsoContains: "Limited quantities", expectsImage: true },
...
for (const r of routes) {
  try {
    ...
    if (r.alsoContains && !body.includes(r.alsoContains)) {
      console.error(`  FAIL  content missing "${r.alsoContains}"  ${r.path}`);
      failures++;
      continue;
    }
