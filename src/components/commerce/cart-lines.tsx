...
import {
  ...
  type CartUpdateState,
} from "@/server/actions/cart";

const initialUpdateState: CartUpdateState = {};
  ...
  return (
    <>
      {updateState.error && (
        <div
          role="alert"
          className="mb-4 border border-burgundy bg-bone-50 p-4 text-sm"
        >
          {updateState.error}
        </div>
      )}
          ...
          <li key={line.id} className="flex gap-4 py-6">
            ...
            <div className="flex flex-1 flex-col justify-between">
              ...
              <div className="mt-3 flex items-center justify-between">
                <form action={updateAction} className="flex items-center border border-ink-20">
                  ...
                  <button
                    ...
                    className="h-9 w-9 hover:bg-bone-200 disabled:opacity-50"
                    aria-label="Decrease"
                  >
                    -
                  </button>
                  ...
                  <button
                    ...
                    name="quantity"
                    value={line.quantity + 1}
                    disabled={updatePending || line.quantity >= line.variant.inventory}
                    className="h-9 w-9 hover:bg-bone-200 disabled:opacity-50"
                    aria-label="Increase"
                  >
                    +
